...
You can run r code in the `Console` but once you close RStudio, you loss your code in `Console`. 
...
In an `R Script` you can write code and add comments. You can run each line by pressing `ctrl + enter/return`. 
...
To check what packages you already have, go to the `Packages` window on the bottom right. 
...
The first way is by clicking. Open the `Pacakges` window on the bottom right, click on `Install` `r icons::fontawesome("arrow-circle-down", style = "solid")`. 
