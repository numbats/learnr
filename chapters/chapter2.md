...
-   Construct a matrix *M* by stacking *S* on top of *T*,
    i.e. $M = \\begin{bmatrix}S\\\\T\\end{bmatrix}$. Assign your result
    to the variable `M`.
...
first\_ten\_element data frame in ascending order of `atomic_no`, assign
...
weekly\_spending) to the variable `bank_balance` until `bank_balance` is
